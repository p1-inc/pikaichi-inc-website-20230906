			...
			if (user) {
				...
				if (user.emailVerified && (token?.claims.role === "super" || token?.claims.role === "admin")) {
					...
					if (!hasVisited) {
						...
						// セッションストレージに 'visited' をセットして、再度このページを訪れた際にはコンポーネントを表示しないようにする
	...
	return (
		<div>
			...
			<AuthWrapper loading={loading} authUser={authUser}>
				<StartUkimaMessage displayUkimaMessage={displayUkimaMessage} setDisplayUkimaMessage={setDisplayUkimaMessage} />
